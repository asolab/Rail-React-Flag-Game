...
export const getQuestions = createAsyncThunk('questions/getQuestions', async (difficulty)=>{
    try{
        const resp = await fetch(`http://localhost:3000/v2/fuests?limit=15&difficulties=${difficulty}`)
        return resp.json()
    }catch(err){
        console.log(err);
    }
    ...
    reducers:{
        ...
        nextQuestion: (state)=>{
            ...
            let newArray = {id: temp.id, 
              ...
              image: temp.image,
              options: shuffleArray([...temp.incorrectAnswers, temp.correctAnswer])  /* preceding options */
            }
            state.index += 1
