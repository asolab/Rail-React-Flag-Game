...
export const getQuestions = createAsyncThunk('questions/getQuestions', async (difficulty)=>{
    try{
        const resp = await fetch(`http://localhost:3000/v2/questions?limit=15&difficulties=${difficulty}`)
        return resp.json()
    }catch(err){
        console.log(err);
    }
    ...
    reducers:{
        ...
        nextQuestion: (state)=>{
            ...
            let newArray = {id: temp.id, 
              ...
              options: shuffleArray([...temp.incorrectAnswers, temp.correctAnswer])  /* preceding option */
            }
            state.index += 1
